...
public class TaskAuditRepositoryTests
{
    ...
    {
        // Arrange
        var mock = new Mock<ITaskAuditRepository>();

        mock.Setup(repo => repo.Add(It.IsAny<TaskAuditEntity>())).ReturnsAsync(true);

        var service = mock.Object;

        // Act
        var result = await service.Add(new TaskAuditEntity());
    ...
    public async Task Update_ShouldThrowException()
    {
        ...
        mock.Setup(repo => repo.Update(It.IsAny<TaskAuditEntity>()))
            .ThrowsAsync(new Exception("Erro ao atualizar"));

        var repo = mock.Object;

        // Act & Assert
        var ex = await Assert.ThrowsAsync<Exception>(() => repo.Update(new TaskAuditEntity()));
