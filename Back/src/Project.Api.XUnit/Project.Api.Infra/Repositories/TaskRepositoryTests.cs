...
public class TaskRepositoryTests
{
    ...
    public async Task Add_ShouldReturnTrue()
    {
        ...
        mock.Setup(repo => repo.Add(It.IsAny<TaskEntity>())).ReturnsAsync(true);

        var service = mock.Object;

        // Act
        var result = await service.Add(new TaskEntity());
    ...
    public async Task Update_ShouldThrowException()
    {
        ...
        mock.Setup(repo => repo.Update(It.IsAny<TaskEntity>()))
            .ThrowsAsync(new Exception("Erro ao atualizar"));

        var repo = mock.Object;

        // Act & Assert
        var ex = await Assert.ThrowsAsync<Exception>(() => repo.Update(new TaskEntity()));
